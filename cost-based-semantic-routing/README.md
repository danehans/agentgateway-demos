...
become healthy. `setup` makes no OpenAI requests. `all` sends 102 small billable
requests by default: two routing probes, four smoke-test requests, and 96
primary requests.

The dataset has 48 concise Go and Rust developer prompts: 24 routine
implementation or test tasks and 24 complex correctness or distributed-systems
...
With `all --yes`, this cache evaluation sends 30 billable requests: two routing
probes, four smoke-test requests, and 24 primary requests across the two lanes.
...
The checked-in cache-transition dataset has two six-turn conversations. Each
