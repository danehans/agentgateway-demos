...
class EvaluationToolingTest(unittest.TestCase):
    ...
    def test_default_dataset_has_expected_model_mix(self):
        ...
        self.assertEqual(
            sum(row["expected_model"] == "gpt-5.4-nano" for row in rows), 24
        )
        self.assertEqual(
            sum(row["expected_model"] == "gpt-5.5" for row in rows), 24
        )
        self.assertEqual(
            {row["language"] for row in rows}, {"go", "rust"}
        )
        self.assertEqual(sum(row["language"] == "go" for row in rows), 24)
        self.assertEqual(sum(row["language"] == "rust" for row in rows), 24)
    ...
    def test_cache_transition_dataset_has_long_ordered_conversations(self):
        ...
        for conversation_id in {row["conversation_id"] for row in rows}:
            ...
            self.assertEqual([row["turn"] for row in turns], [1, 2, 3, 4, 5, 6])
